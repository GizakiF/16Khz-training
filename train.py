...
outFolder = '../output/StratifiedKFold'
os.makedirs(outFolder, exist_ok=True)

# location of data.pkl
dataFolder = '../dataset/osf/'
...
# Initialize all_reports list BEFORE the loop
all_reports = []
...
for i, iSubject in enumerate(subjectNbTab):
    ...
    thresh = cm.max() / 2.  # Calculate threshold for color contrast
    rows, cols = cm.shape  # Unpack the tuple into individual variables

    for i_cm_row in range(rows):  # Iterate through rows
        for j_cm_col in range(cols):  # Iterate through columns
    ...
    all_reports.append(df)  # Append the df to the list in each iteration
...
# Use labels from the last subject (should be the same for all)
unique_labels = np.unique(Y_test)
tick_positions = np.arange(len(unique_labels))
...
for i_avg_cm_row in range(rows):
    for j_avg_cm_col in range(cols):
        plt.text(j_avg_cm_col, i_avg_cm_row, format(average_cm[i_avg_cm_row, j_avg_cm_col], 'd'),
                 horizontalalignment="center",
                 color="white" if average_cm[i_avg_cm_row, j_avg_cm_col] > thresh else "black")

plt.tight_layout()
plt.savefig(os.path.join(outFolder, "average_confusion_matrix.png"))
plt.close()

# ***SAVE ALL REPORTS AFTER THE LOOP***
# concatenating all dataframes into one
all_reports_df = pd.concat(all_reports, keys=subjectNbTab)
# saving the final dataframe
all_reports_df.to_csv(f"{outFolder}/classification_report_all_subjects.csv")
# Summary statistics
print("\n=== Summary ===")
