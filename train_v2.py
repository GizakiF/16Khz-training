...
for i, iSubject in enumerate(subjectNbTab):
    ...
    required_columns = ['accuracy', 'macro avg', 'weighted avg']
...
# Format the DataFrame to match the desired CSV structure
all_subjects_results.rename(columns={
    'precision': 'post_precision',
    'recall': 'post_recall',
    'f1-score': 'post_f1-score',
    'support': 'post_support'
}, inplace=True)

# Add columns for pre_precision, pre_recall, pre_f1-score, pre_support if they exist
if 'pre_precision' not in all_subjects_results.columns:
    all_subjects_results['pre_precision'] = None
if 'pre_recall' not in all_subjects_results.columns:
    all_subjects_results['pre_recall'] = None
if 'pre_f1-score' not in all_subjects_results.columns:
    all_subjects_results['pre_f1-score'] = None
if 'pre_support' not in all_subjects_results.columns:
    all_subjects_results['pre_support'] = None

# Ensure the subject column is named correctly
...
all_subjects_results = all_subjects_results[columns_order]

# Save the formatted DataFrame to a CSV file
all_subjects_results.to_csv(os.path.join(
    outFolder, "all_subjects_results_formatted.csv"), index=False)
